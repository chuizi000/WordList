...
static void OpenButtonMonitor(Core& core, Init& init, char* text) {
    ...
    if (strlen(openDlg.getChosenPath())>0) {
        ...
        FILE* fp = fopen(openDlg.getChosenPath(), "r");
        int i = 0;
        while ((text[i] = fgetc(fp)) && i < 1024 * 16) {
            i ++;
        }
        //fgets(text, 1024 * 16, fp);
        fclose(fp);
    } else {
...
static void SaveButtonMonitor(Core& core, Init& init, char* text) {
    ...
    if (strlen(saveDlg.getChosenPath())>0) {
        ...
        FILE* fp = fopen(saveDlg.getChosenPath(), "w");
        fprintf(fp, "%s", text);
        fclose(fp);
    } else {
...
int main(int, char**)
{
    ...
    GLFWwindow* window = glfwCreateWindow(540, 360, "Longest Word List", NULL, NULL);
    if (window == NULL)
        return 1;
    ...
    ImVec2 window_size = ImVec2(540.0f, 360.0f);
